...
**Internal QA pass — external review pending. 2026-05-29 PM update: n=7 statistical certification landed.** As of commit `ca32fcd`, the area-chair sniff test produced a `WEAK_REJECT` verdict (see [`audits/REVIEWER_PASS_PAPER.md`](audits/REVIEWER_PASS_PAPER.md)). This revision (commit forthcoming) addresses every BLOCKER and every MAJOR finding from that pass. The strategic decision adopted in this revision is **protocol-as-contribution (framing (a) per area-chair item #1)**: the dual-track audit + Fixer + per-experiment-page machinery is the headline contribution, and the three Phase-8 numerical results (`pair_gm_pdw`, `slot_act_sine`, `sg_only_phi_budget` post-fix) are presented as **illustrative case studies of protocol output that, as of the 2026-05-29 n=7 extension, are now also FORMALLY CERTIFIED at α=0.05 under Holm-Bonferroni** (paired Wilcoxon p=(1/2)^7=0.0078 < α'=0.0167 for each winner; see [`paper/STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §0 promotion banner). These are the project's first formally-certified empirical claims at NeurIPS-standard α — pending the hill-climb at converged training budget (Phase-9a, still open). The previous "Reviewer-acceptance ACCEPT verdict at commit `0343f35`" banner was self-graded and has been removed.
...
| 8 | BLOCKER | Single-seed limitation buried | **DONE** (moved to abstract; further: as of 2026-05-29 the three Phase-8 winners now carry **n=7 seeds** on CIFAR-100 30-ep, the rest of the screen retains single-seed status by design) |
...
We present a **methodological contribution: a dual-track skeptical audit + Fixer + per-experiment-page protocol** for autoresearch campaigns that propose large families of architectural priors. The protocol layers two independent expert teams on top of an implementation campaign — an 8-agent **implementation-critic team** that verifies code-vs-doc correspondence with mechanism-pinning tests, and an 8-agent **research-scientist-critic team** that challenges scientific merit independent of code correctness — followed by an 8-agent **Fixer campaign** that patches confirmed defects with mechanism-verifying tests, then **re-runs every affected sweep row** on the corrected code. We motivate, specify, and operationalise the protocol in [`CLAUDE.md`](CLAUDE.md) Rules 20–28 and package it as seven content-agnostic skills in [`skills/`](skills/). The protocol is the contribution. As empirical **calibration**, we apply it to an 84-hypothesis nature-inspired neural-network design space (φ-scaling, hexagonal lattices, Platonic / icosahedral equivariance, fractal recursion, toroidal closure, Chladni cymatic init, golden-angle modulation, and 15 cross-paradigm hybrids) on CIFAR-10 / CIFAR-100. The calibration reports: of 83 implementations audited, 51 % land non-PASS (3 BROKEN / 15 MAJOR / 24 MINOR); of 81 hypotheses scientifically critiqued, only ONE ranks NOVEL+TESTABLE; the Fixer campaign produced 8 commits and ~34 new mechanism-verifying tests; post-fix re-runs identified three candidates that pass the project's worst-leader-seed > best-baseline-seed CIFAR-100 screening gate. **As of 2026-05-29, these three were extended to n=7 seeds and now FORMALLY CERTIFY at α=0.05 under Holm-Bonferroni**: `pair_gm_pdw` Δmean=+1.74 pp (95% CI [+1.42, +2.09]), `slot_act_sine` Δmean=+1.78 pp (95% CI [+1.38, +2.18]), `sg_only_phi_budget` Δmean=+1.24 pp (95% CI [+0.84, +1.67]); paired Wilcoxon W=0, exact one-sided p=(1/2)^7=**0.0078 < α'_Holm=0.0167** for each winner across the k=3 family (see [`paper/STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md)). These are the project's first formally-certified empirical claims at NeurIPS-standard α — promoted from SCREENING to EVALUATION tier under Rule 28. We frame them as **case studies of protocol output that DID survive Phase-9-level statistical rigor at the screening compute budget**, and note that the per-hypothesis hill-climb at the converged training budget (Phase 9a) and the non-φ 3-axis control row (§5.5.1) remain open.

**Three calibration caveats are stated up front.** (a) **Comparison baseline is 6.5 pp below SOTA.** ResNet-20 at 164 ep reaches ~91.25 % top-1 on CIFAR-10; our 12-ep screening baseline is ~84.78 %. The n=7 certification of the three Phase-8 winners holds AT THIS BUDGET (12-ep CIFAR-10 and 30-ep CIFAR-100); whether the lift survives the converged 164-ep training condition is a Phase-9a question. (b) **Most single-prior sweep rows remain single-seed.** As of the 2026-05-29 n=7 extension, the three certified winners (`pair_gm_pdw`, `slot_act_sine`, `sg_only_phi_budget`) carry **n=7 seeds** on CIFAR-100 30-ep with paired Wilcoxon p=0.0078 clearing Holm-Bonferroni α'=0.0167; the rest of the 35-row CIFAR-10 screen is still n=1 by design (screening tier). (c) **Implementer, audit, sci-critic, and Fixer agents are all from the same model family** (Claude Opus 4.7). The audit's "independence" is methodological theatre at the model-family level; the 51 % non-PASS rate is conditional on this caveat and has not been calibrated against a known-good third-party codebase.
...
### 5.5 · Three Phase-8 candidates surfaced by the protocol — CERTIFIED at n=7 (2026-05-29)

**Framing (per area-chair item #1).** This subsection reports three sweep-tag outcomes that pass the project's CIFAR-100 screening gate AND, as of the 2026-05-29 n=7 extension, also pass the formal paired-Wilcoxon test at α=0.05 under Holm-Bonferroni correction across the k=3 family. The framing remains **case studies of protocol output** — but they are now case studies of protocol output that DID survive Phase-9-level statistical rigor at the screening compute budget. Whether they also survive a per-hypothesis hill-climb at converged training budget (Phase 9a, ahead) and the non-φ 3-axis control row (§5.5.1) remains open.

**Three protocol-screened candidates (7-seed CIFAR-100, 30 ep, post-fix code; sweep completed 2026-05-29 PM):**

| tag | C100 mean (n=7) | min leader seed | baseline max seed | Δmean vs baseline | statistical support |
|---|---:|---:|---:|---:|---|
| `pair_gm_pdw` (H09+H48+H44 orthogonal stack) | 0.5786 | 0.5761 | 0.5662 | **+1.74 pp** | W=0, exact one-sided p=(1/2)^7=**0.0078**; bootstrap 95% CI on Δmean [+1.42, +2.09] pp **excludes 0**; **CLEARS Holm-Bonferroni α'=0.0167** |
| `slot_act_sine` (H81 SIREN, single prior) | 0.5790 | 0.5725 | 0.5662 | **+1.78 pp** | W=0, exact one-sided p=**0.0078**; bootstrap 95% CI on Δmean [+1.38, +2.18] pp **excludes 0**; **CLEARS Holm-Bonferroni α'=0.0167** |
| `sg_only_phi_budget` (H09, post-fix 1:1.623:2.629) | 0.5736 | 0.5686 | 0.5662 | **+1.24 pp** | W=0, exact one-sided p=**0.0078**; bootstrap 95% CI on Δmean [+0.84, +1.67] pp **excludes 0**; **CLEARS Holm-Bonferroni α'=0.0167** |
| `baseline_resnet20` (rail) | 0.5612 | — | — | — | (reference, n=7, σ = 0.451 pp on CIFAR-100) |

All three Phase-8 rows produce 7/7 positive paired deltas — the negative-rank sum W = 0 — so the exact one-sided Wilcoxon p achieves its n=7 floor of (1/2)^7 = **0.0078**. This is **strictly less than the Holm-Bonferroni-corrected α'=0.05/3 = 0.0167** demanded for the smallest test of a k=3 family at family-wise α=0.05. Under step-down monotonicity (0.0078 < 0.025 < 0.05), the second and third tests also clear their corresponding step-down thresholds. **All three Phase-8 winners therefore CLEAR Holm-Bonferroni at α=0.05.** The 95% bootstrap CIs on Δmean exclude 0 by margins of ≥2σ_baseline, and the CI widths are approximately half their n=3 predecessors (variance scales as 1/n, width as 1/√n).

This is the project's first formally-certified empirical claim at NeurIPS-conventional α. Per CLAUDE.md Rule 28, the three winners are PROMOTED from **SCREENING** to **EVALUATION** tier. The Phase-5 ordinal gate min(leader_s) > max(baseline_s) at n=7 is equivalent to a paired sign test at α=(1/2)^7=0.0078 (verified holding for all three winners), now **MORE stringent** than α=0.05 — a planning win for the protocol design. The complete formal Wilcoxon + Holm-Bonferroni + bootstrap-CI analysis is in [`paper/STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §0 (promotion banner) and §1 (per-claim certification table).

**Honest reading of the three case studies (post-n=7 certification):**
- `pair_gm_pdw` stacks three orthogonal axes (architecture / momentum / weight-decay); Δmean +1.74 pp is now CERTIFIED at α=0.05 under Holm-Bonferroni (n=7, p=0.0078). The certification establishes the lift is real at the screening compute budget; it does NOT resolve the **non-φ 3-axis regularizer control** confound (§5.5.1) — we still cannot attribute the lift to φ-content vs "any three regularisation axes tuned away from baseline defaults" without that control.
- `slot_act_sine` swaps ReLU for `sin(ω·x)` with ω=1; Δmean +1.78 pp is now CERTIFIED at α=0.05 under Holm-Bonferroni (n=7, p=0.0078). Certification establishes that periodic activation at this recipe produces a real lift; the **non-φ activation control sweep** (`tanh`, `softplus`, `gelu`, `swish` at the same SLOT recipe) is still outstanding. Per Sitzmann et al. (NeurIPS 2020), SIREN's activation-engineering signal has no φ content; certification is consistent with "SIREN works at the screening budget", not specifically with "φ-flavoured SIREN works."
- `sg_only_phi_budget` post-fix Δmean +1.24 pp is now CERTIFIED at α=0.05 under Holm-Bonferroni (n=7, p=0.0078). The sci-critic verdict remains DERIVATIVE+TESTABLE (rediscovery of RegNet's Pareto-optimal width-progression region — Radosavovic et al. CVPR 2020 explicitly prefers `w_m ∈ [2.5, 2.9]`, NOT φ=1.618). A tuned-RegNetX-200MF baseline (item #13) would settle whether the lift survives a tuned-baseline upgrade; that comparison is deferred to Phase-9a.
...
### 5.8 · Audit calibration on third-party code (area-chair item #12, RESOLVED)

The 51 % impl-critic non-PASS rate (§5.1) is presented as protocol calibration in this paper. The area-chair correctly observed that without a **false-positive baseline** measurement — the same audit team applied to a known-good third-party codebase — the 51 % is uninterpretable: a high non-PASS rate is observationally consistent with both "high defect density in our codebase" and "high false-positive rate in our audit team."

**The calibration has now been done.** Track-A doctrine was applied to a 15-hypothesis sample drawn from `pytorch/vision` (`ResNet`/`BasicBlock`/`Bottleneck`/`Wide-ResNet`, `DenseNet`, `VGG`, `SqueezeNet`, `MobileNetV2`) and the cited `pytorch/pytorch` core modules (`torch.optim.Adam`, `torch.optim.SGD`, `torch.nn.init.kaiming_normal_`, `torch.optim.lr_scheduler.CosineAnnealingLR`, `torch.nn.BatchNorm2d`); HEAD of `main` snapshot taken 2026-05-30. The full audit (with file:line citations for every verdict) is in [`audits/AUDIT_CALIBRATION_THIRD_PARTY.md`](audits/AUDIT_CALIBRATION_THIRD_PARTY.md); the verbatim third-party code chunks are in [`audits/AUDIT_CALIBRATION_RAW_NOTES.md`](audits/AUDIT_CALIBRATION_RAW_NOTES.md).

**Headline result:** **10 PASS, 5 MINOR, 0 MAJOR, 0 BROKEN. Non-PASS rate 33.3 %.** Compared to our project's 50.6 % (29 % MINOR, 18 % MAJOR, 4 % BROKEN of 83), the calibration validates the area-chair's concern *at the MINOR tier* (29 % project vs 33 % calibration ≈ comparable) but NOT *at the MAJOR/BROKEN tier* (22 % project vs 0 % calibration). The 5 third-party MINORs are documented intentional deviations — V1.5 stride placement (`Bottleneck`), PyTorch Nesterov reformulation (`SGD`), inverted BN momentum semantics (`BatchNorm2d`), no-restart "cosine annealing" (`CosineAnnealingLR`), and the WRN-Bottleneck reuse of the WRN-d-k name (`wide_resnet50_2`). None are MAJOR/BROKEN.

**Implication for §5.1 framing:** the audit's diagnostic power is **partially overstated when reporting "51 % non-PASS" as a single number**. The credible audit signal is the **22-pp excess in the MAJOR/BROKEN tier** (22 % project minus 0 % calibration floor); the MINOR-tier excess (29 % minus 33 % = −4 pp) is auditor aggressiveness in line with the calibration baseline. The MAJOR/BROKEN tier IS where the H09 phi_budget realised-ratio drift, the G3 H21 undocumented hex_phi divergence, and the G7 H67 broken GoldenRoPE import live — concrete defects an unaudited pipeline would have shipped. Caveat: with n=15 calibration vs n=83 project the two-proportion chi-squared p ≈ 0.22, so the aggregate-rate difference is not statistically significant at α=0.05; the MAJOR/BROKEN sub-tier comparison (15/83 = 18 % vs 0/15 = 0 %; one-sided Fisher exact p ≈ 0.07) is the more credible split, marginal but in the direction of "real codebase excess defect density."

**Recommended camera-ready re-framing of §5.1:** report both the aggregate 51 % AND the MAJOR/BROKEN-tier 22 %, cite [`audits/AUDIT_CALIBRATION_THIRD_PARTY.md`](audits/AUDIT_CALIBRATION_THIRD_PARTY.md) as the false-positive baseline, and state that the 22-pp MAJOR/BROKEN-tier excess over the 0 % calibration floor is the diagnostically-credible signal, not the aggregate 51 %. The protocol-as-contribution claim is unchanged in load-bearing strength: the audit DID surface the MAJOR/BROKEN-tier defects that an unaudited pipeline would have shipped, and the calibration shows this tier is empty on a peer-trusted reference codebase. Phase-9b future work: extend the calibration sample to 50-100 hypotheses spanning timm + Hugging Face transformers + Lightning Bolts to tighten the CI.

## 6 · Empirical evidence (CIFAR-10 / CIFAR-100) — protocol output

### 6.1 · Pre-fix screening sweep (single seed, 12 epochs)

Across 35 single-prior sweep rows on CIFAR-10 at seed 0 / 12 epochs, the only variant beating the ResNet-20 baseline (84.78 %) was **H09 phi_budget at 85.54 %** (+0.76 pp). **All 35 rows are n=1; the 51 %-non-PASS rate is therefore screened on a single-seed sweep and the per-row deltas sit well inside the empirical CIFAR-10 12-ep seed-noise band.** Per [`STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §4.1, the **pooled CIFAR-10 12-ep σ across 11 multi-seed tags is 0.607 pp**, so **2σ_pooled ≈ 1.21 pp**; the project's prior "±0.5 pp = within seed noise" rule of thumb (§7.3.1 below) is OPTIMISTIC — even the **99th-percentile single-seed Δ across the 58 seed-0 non-baseline tags is +0.96 pp, still INSIDE the 2σ band**, so no single CIFAR-10 12-ep seed-0 tag has prima-facie statistical credibility as a positive (§4.2 of `STATISTICAL_TESTS.md`). The pre-fix `golden_adam` row at 51.96 % (−32.82 pp) was originally framed as the campaign's clean falsification, **but Track A's H41 audit + the post-fix re-run requalified that headline**: the catastrophic collapse was driven by `eps = 1/φ⁴ ≈ 0.146` (which dominated Adam's denominator at CIFAR gradient scales ~1e-3, inflating effective LR ~6.85×), NOT by the β-shift. After Fixer-Opt restored stock `eps = 1e-8` and kept only the φ-defaults for β, post-fix CIFAR-10 12-ep top-1 = 0.8394, Δ ≈ −1 pp vs baseline. Reddi 2018's non-convergence proof for β2 < 0.95 is asymptotic and does not yet bite at 12 ep — the β-only regression is mild, not catastrophic. **H41 is therefore re-classified as WEAKLY NEGATIVE at 12-ep CIFAR-10 screening, with the clean β-only Reddi-regime falsification deferred to the Phase-9 hill-climb at varied β2 + longer training horizons (100+ ep).** The pre-fix 0.5196 row is retired as eps-confounded. The case-study reading: the protocol's screening sweep correctly surfaced a candidate that warranted Phase-9, AND identified an implementation confound (eps) that an unaudited pipeline would have published as a "clean β-only falsification."
...
The Phase-8 `pair_gm_pdw` row stacks exactly three orthogonal axes (architecture + momentum + weight-decay) and lands at 85.85 % CIFAR-10 12-ep — the best single combo in the post-fix sweep. The 7-seed CIFAR-100 30-ep mean (0.5786, Δmean +1.74 pp) is **CERTIFIED at α=0.05 under Holm-Bonferroni** (paired Wilcoxon p=0.0078, 2026-05-29 extension). This is the protocol's nominee for the "three-axis orthogonal stack outperforms its solo components" hypothesis — certified at the screening compute budget; the φ-content vs non-φ-3-axis attribution question is still resolved by the §5.5.1 non-φ control and the Phase-9a hill-climb.
...
- **Multiple-comparisons correction now formally applied AND CLEARED for the Phase-8 family** (per [`paper/STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §0–§3). For the 35-row CIFAR-10 screen at n=1 per row, the smallest achievable paired p-value is 0.5 and the per-test Bonferroni-corrected α' = 0.05/35 ≈ 0.00143, so **no CIFAR-10 screening row can clear ANY corrected α at n=1** — the screen is exploratory by mathematical necessity (vindicating §7.3.1's reclassification, modulo the HARKing concession). For the Phase-8 CIFAR-100 family at k=3 simultaneous tests, Holm-Bonferroni requires α' ≤ 0.0167. The 2026-05-29 n=7 extension produced 7/7 positive paired deltas on each of the three winners, yielding exact one-sided paired Wilcoxon p=(1/2)^7=**0.0078 < α'=0.0167 → all three winners CLEAR Holm-Bonferroni**. The previously-scheduled Phase-9 n=7 re-run is therefore complete and the three winners are PROMOTED from SCREENING to EVALUATION tier under Rule 28.
...
- **3-seed re-runs at the best config found.** Single-seed differences on CIFAR-10 12-ep are within seed noise up to **2σ_pooled ≈ 1.21 pp** (pooled empirical σ = 0.607 pp across 11 multi-seed tags; the previous "±0.5 pp" rule of thumb is OPTIMISTIC — see [`STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §4.1). Phase-9 hill-climb requires **n ≥ 7 seeds** at the best config to clear Holm-Bonferroni α'=0.0167 across the k=3 Phase-8 family.
...
- Statistical-test analysis (Wilcoxon + Holm-Bonferroni + bootstrap CIs): [`STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) (parallel work-stream)
...
**F6 · Multiple-comparisons correction missing.** Engaged in §7.3 Limitations and §7.4-8 Future Work; spliced from [`paper/STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) into §5.5 table. **Post-n=7 headline result (2026-05-29):** paired Wilcoxon W=0, exact one-sided p=(1/2)^7=**0.0078** on each of the three Phase-8 rows; bootstrap 95% CIs on Δmean exclude 0 ([+1.42, +2.09] pp for `pair_gm_pdw`, [+1.38, +2.18] for `slot_act_sine`, [+0.84, +1.67] for `sg_only_phi_budget`); **ALL THREE CLEAR Holm-Bonferroni α'=0.05/3=0.0167**. The 35-row CIFAR-10 screen at n=1 still cannot clear ANY Bonferroni-corrected α (smallest achievable p=0.5; per-test α'=0.00143) — by design (screening tier). The Phase-9 n=7 extension is the minimum sample size that makes the headline claims defensible under Holm; it landed cleanly on the date noted.
...
| L130/134/138 (Phase-8 numbers, "outside seed noise") | BLOCKER #3 | §5.5 (Wilcoxon W=0/p=0.0078 + bootstrap CI + Holm-Bonferroni α'=0.0167 spliced from `paper/STATISTICAL_TESTS.md`; **all three CLEAR α=0.05 under Holm-Bonferroni at n=7**, 2026-05-29 promotion) |
