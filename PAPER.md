...
| 3 | BLOCKER | Statistical unsupport on Phase-8 numbers | **DONE** — Wilcoxon W=0 / p_one=0.125 (n=3 exact-distribution floor), bootstrap 95% CIs, and Holm-Bonferroni α'=0.0167 (all FAIL) spliced into §5.5 from [`STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md); n ≥ 7 seeds required for any single Phase-8 row to clear Holm at α=0.05 across the k=3 family |
...
All three Phase-8 rows produce the **theoretical-minimum** paired one-sided Wilcoxon p at n=3, namely (1/2)^3 = 0.125, because every per-seed delta is strictly positive (negative-rank sum W = 0). This is the strongest possible result the n=3 design can produce, and it is still **2.5× looser than α=0.05** and **7.5× looser than Holm-Bonferroni α'=0.05/3=0.0167** across the k=3 family. The 95% bootstrap CIs on Δmean all exclude 0 by comfortable margins (lower-bound at least 1.5× the CIFAR-100 baseline σ=0.248 pp); the effect-size signal is real-looking, but the n=3 sample is structurally insufficient for any α=0.05 single-test rejection — **per [`STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §3.3, n ≥ 7 seeds is the minimum required for paired Wilcoxon to clear Holm-Bonferroni α'=0.0167 across the k=3 Phase-8 family**, even when every paired delta is positive.

The "min-leader-seed > max-baseline-seed" gate is an ordinal one-sided non-parametric construction with low formal power; characterised as a paired one-sided sign test on n=3 seed-aligned seeds, its α is exactly (1/2)^3 = 0.125. The gate's role is to **screen** candidates worth Phase-9 evaluation, NOT to declare empirical wins; per [`STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §2, the Phase-5 ordinal gate is therefore equivalent to a one-sided sign test at α≈0.125, NOT at α=0.05. The complete formal Wilcoxon + Holm-Bonferroni + bootstrap-CI analysis is now in [`STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) and is spliced into the table above; the three "winners" are candidates with non-zero-excluding bootstrap CIs but **formally uncertified at n=3**.
...
2. **Architectural-level (PROVISIONAL):** The post-fix architecture lifts the CIFAR-100 3-seed median by +0.89 pp at the 30-ep screening budget; the proper Δmean estimate is **+0.91 pp** with bootstrap 95% CI **[+0.44, +1.36] pp** (lower bound 1.5× the CIFAR-100 baseline σ=0.248 pp; 0 is excluded). The often-quoted **+0.25 pp** "lead floor" (min-leader-seed − max-baseline-seed = 0.5687 − 0.5662) is the worst-case ordinal margin under the Phase-5 gate at α=(1/2)^3=0.125, **NOT** a difference-of-means estimate; per [`STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §5.3, the +0.25 pp number is conflated with Δmean in the pre-revision paper and is labelled accordingly here. The paired Wilcoxon p_one=0.125 is the n=3 floor and FAILS α=0.05 single-test rejection (let alone Holm-Bonferroni α'=0.0167 across the k=3 family). The pre-fix median was ~0.6 pp HIGHER than the post-fix median, consistent with "the broken realised ratio happened to land a fortuitously-high seed-0 result." Whether the +0.91 pp Δmean represents a genuine architectural signal at the literature-canonical 164-ep / tuned-baseline regime is decided in Phase-9 hill-climb at **n ≥ 7 seeds** + the tuned-RegNet head-to-head (§7.4-4), not in this submission.
...
- **Multiple-comparisons correction now formally applied** (per [`STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §3). For the 35-row CIFAR-10 screen at n=1 per row, the smallest achievable paired p-value is 0.5 and the per-test Bonferroni-corrected α' = 0.05/35 ≈ 0.00143, so **no CIFAR-10 screening row can clear ANY corrected α at n=1** — the screen is exploratory by mathematical necessity (vindicating §7.3.1's reclassification, modulo the HARKing concession). For the Phase-8 CIFAR-100 family at k=3 simultaneous tests, Holm-Bonferroni requires α' ≤ 0.0167, but the n=3 exact-distribution paired-Wilcoxon floor is p_one=0.125; **all three Phase-8 winners FAIL Holm-Bonferroni**, and **n ≥ 7 seeds is required** for paired Wilcoxon to clear Holm at α=0.05 across k=3 (P(all 7 +) = 1/128 = 0.0078 < 0.0167). Phase-9 re-runs at n ≥ 7 are scheduled (§7.4-1).
...
**F6 · Multiple-comparisons correction missing.** Engaged in §7.3 Limitations and §7.4-8 Future Work; **now spliced from [`STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) into §5.5 table**. Headline result: paired Wilcoxon W=0 / p_one=0.125 (n=3 exact-distribution floor) on each of the three Phase-8 rows; bootstrap 95% CIs on Δmean exclude 0 by comfortable margins ([+1.11, +1.63] pp for `pair_gm_pdw`, [+1.13, +1.67] for `slot_act_sine`, [+0.44, +1.36] for `sg_only_phi_budget`); ALL THREE FAIL Holm-Bonferroni α'=0.05/3=0.0167. The 35-row CIFAR-10 screen at n=1 cannot clear ANY Bonferroni-corrected α (smallest achievable p=0.5; per-test α'=0.00143). Phase-9 at **n ≥ 7 seeds** is the minimum sample size required to make headline claims potentially defensible under Holm.
...
| L130/134/138 (Phase-8 numbers, "outside seed noise") | BLOCKER #3 | §5.5 (Wilcoxon W=0/p=0.125 + bootstrap CI + Holm-Bonferroni α'=0.0167 spliced from `STATISTICAL_TESTS.md`; all three FAIL formal α=0.05) |
