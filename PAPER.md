...
| 3 | BLOCKER | Statistical unsupport on Phase-8 numbers | **IN PROGRESS** — references new `STATISTICAL_TESTS.md` (parallel agent); TODO placeholders in-place where the file is not yet populated |
...
|---|---:|---:|---:|---:|---|
| `pair_gm_pdw` (H09+H48+H44 orthogonal stack) | 0.5786 | 0.5761 | 0.5662 | +1.34 pp | `[STAT_TEST_RESULT — see STATISTICAL_TESTS.md when populated]` |
| `slot_act_sine` (H81 SIREN, single prior) | 0.5784 | 0.5766 | 0.5662 | +1.32 pp | `[STAT_TEST_RESULT — see STATISTICAL_TESTS.md when populated]` |
| `sg_only_phi_budget` (H09, post-fix 1:1.623:2.629) | 0.5741 | 0.5687 | 0.5662 | +0.89 pp | `[STAT_TEST_RESULT — see STATISTICAL_TESTS.md when populated]` |
| `baseline_resnet20` (rail) | 0.5652 | — | — | — | (reference, n=3, σ = 0.0020) |

<!-- TODO(Reviewer-Followup): replace [STAT_TEST_RESULT] placeholders with paired Wilcoxon W, p-value, Holm-Bonferroni adjusted p across the 3-row family, and bootstrap 95% CI on Δ once STATISTICAL_TESTS.md is populated by the Stat-Tests-Adder agent. -->

The "min-leader-seed > max-baseline-seed" gate is an ordinal one-sided non-parametric construction with low formal power; characterised as a one-sided sign test on a 3v3 sample, its α is ~0.125. The gate's role is to **screen** candidates worth Phase-9 evaluation, NOT to declare empirical wins. Per area-chair item #3, the formal Wilcoxon + Holm-Bonferroni + bootstrap-CI analysis is in [`STATISTICAL_TESTS.md`](STATISTICAL_TESTS.md) (parallel work-stream); the placeholders here are tracked for the Reviewer-Followup pass.
