...
This paper does two things. **First**, it shows that an LLM-agent autoresearch protocol can be made *self-auditing* in a way that is statistically distinguishable from a clean-code floor: when the same audit doctrine is applied to 62 third-party hypotheses (mainstream research-quality codebases), it returns 0/62 MAJOR/BROKEN; when applied to the protocol's own 84-hypothesis substrate it returns 18/83 MAJOR/BROKEN. The 22-percentage-point excess clears Fisher exact two-sided α=0.05 by a factor of ≈ 2500×. **Second**, it shows the protocol catches its own headline drift: three priors that clear paired Wilcoxon at α'_Holm=0.0167 on n=7 matched-recipe seeds collapse — and are *outperformed by a properly LR-tuned vanilla baseline* — when the baseline is given the same tuning treatment the leaders received.
...
3. **Self-falsification existence proof.** The protocol caught (a) H09 phi_budget's 12.6% realised-stage-ratio drift before any external claim shipped, and (b) its own three-prior matched-recipe headline collapsing under iso-tuned-LR — *before* any "priors help" claim went to publication. This is the load-bearing methodological evidence.
4. **An honestly-demoted 84-hypothesis empirical case study** on CIFAR-10/-100: 51% non-PASS implementation-critic rate; 1/81 NOVEL+TESTABLE sci-critic rate; three Phase-8 candidates pass matched-recipe paired Wilcoxon at Holm-Bonferroni α'=0.0167 across a k=3 confirmatory family; the same three candidates are beaten by an LR-tuned baseline by +2.27 to +2.81 pp at unpaired Mann-Whitney p_one ∈ [0.0083, 0.0111].

What is **explicitly NOT** in the contributions: any claim that the nature-inspired priors broadly outperform mainstream baselines; any claim about transformer-track hypotheses (10/84 untested); any claim about H71 IcosaRoPE3D (the sole NOVEL+TESTABLE survivor is untested); any cross-domain portability claim (open future work).
...
### 5.2 · Catch (b) — the protocol's own three-prior headline collapses under iso-tuned-LR

After the Fixer campaign, the protocol surfaced three Phase-8 candidates that pass a 7-seed CIFAR-100 paired Wilcoxon at α=0.05 under Holm-Bonferroni across a k=3 confirmatory family at the matched-recipe default-config slice (lr=1e-3, wd=5e-4, bs=256, AdamW). All three winners produce 7/7 positive paired deltas → Wilcoxon W=0, exact one-sided p=(1/2)⁷=**0.0078**, clearing Holm-Bonferroni α'=0.0167. Paired-t magnitude p-values sit 3–4 orders below the floor (5×10⁻⁵ to 8×10⁻⁴). This is the **secondary** empirical claim of the paper (§6.5 case study), and the protocol's worked example of what an unaudited pipeline would have published.
...
The tuned-baseline n=3 σ is 0.31 pp — *tighter* than the default-config baseline σ (0.453 pp at n=7), so the diagnostic is not noise-bound. At iso-tuned conditions where the baseline receives the same LR-tuning love that the leaders' hill-climbs gave their priors, **the tuned baseline OUTPERFORMS all three priors by +2.27 to +2.81 pp.** Combined with the Phase-9f n=7 iso-tuned Δ-shrinkage (§6.5) and Phase-9g Controls 1 + 2's partial refutation of the φ-specific and SIREN-specific narratives, **the priors' lift in the default-config cert is substantially explained by baseline-LR-tuning artifact.**

**The protocol caught its own headline drift before any external "priors help" claim shipped.** An unaudited / un-iso-tuned-tested pipeline would have published the default-config n=7 cert as "the priors help" and stopped there. The methodological deliverable is precisely this self-correcting property; it is what we offer the community.
...
**Cross-regime synthesis ([`paper/STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §14, Phase-9h binding diagnostic):** the priors clear the matched-recipe default-config cert at α=0.05 under Holm-Bonferroni, but they do NOT robustly survive a properly LR-tuned baseline. The default-config cert is preserved as a formally-correct matched-recipe statement and as a worked example of protocol output; **the priors are honestly demoted to "screened candidates that do not robustly survive a properly-LR-tuned baseline at NeurIPS-α."** This demotion is the protocol's headline catch.
...
- **Iso-tuned-cell re-certification at n=7 fails for all three winners** (§6.5). The default-config matched-recipe cert is preserved as a formally-correct statement; the iso-tuned cell does not re-certify at NeurIPS-α at this sample size.
...
- **Symmetric iso-tuned n=7 paired close-out (Phase-9i) is filed as future work** to tighten the Phase-9h diagnostic at NeurIPS-α with paired statistics.
- **Auditor catches its own catches only when an iso-tuned diagnostic is run.** The protocol's value depends on actually executing the Phase-9 hill-climb + Controls 1–4 + Phase-9h tuned-baseline binding. The methodology is not self-executing — it has to be invoked.
...
**Second**, the protocol caught its own headline drift. Three default-config-certified Phase-8 winners cleared Holm-Bonferroni α'=0.0167 at matched-recipe n=7 (Wilcoxon p=0.0078 each) — but at iso-tuned conditions where the baseline received the same LR-tuning love the leaders' hill-climbs received, the **tuned vanilla ResNet-20 OUTPERFORMS all three priors by +2.27 to +2.81 pp** (Mann-Whitney p_one ∈ [0.0083, 0.0111]; 95% bootstrap CI lower bound ≥ +1.90 pp; min(tuned) > max(leader) for all three with no rank overlap). The default-config matched-recipe certification is preserved as a worked example and a formally-correct statement; **the priors are honestly demoted to "screened candidates that survive matched-recipe certification but not iso-tuned-LR."** An unaudited / un-iso-tuned-tested pipeline would have published the default-config n=7 cert as "the priors help" and stopped there.

**The protocol's value is precisely that it caught its own at-risk-of-publication headline before any external claim shipped.** The audit + Fixer + per-experiment-page + auto-checkpoint cycle, encoded in CLAUDE.md Rules 20–28 and packaged as seven content-agnostic skills in [`skills/`](skills/), is what we offer the community: portable infrastructure for distinguishing signal from numerology — and for catching one's own LLM-agent autoresearch protocol when it is about to publish a tuning artifact as a "priors help" headline. The cross-domain demonstration of that portability, and a true non-Claude external auditor on the same 10 MAJOR/BROKEN findings, are open future work.
