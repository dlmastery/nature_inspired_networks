...
**Iso-tuned-cell extension at n=3 (Phase-9a baseline-extension, 2026-05-31, n=3 each — [`paper/STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §10).** The Section-7 hill-climb compared the hill-climbed-baseline (bs=256) against hill-climbed-leaders (bs=128). The 2026-05-31 baseline-extension filled out the baseline at the iso-tuned cell (lr=3e-3, wd=5e-4, bs=128, AdamW) to n=3 so that prior-vs-non-prior can be measured at matched bs/lr/wd/optimizer. Iso-tuned baseline mean = 0.5937, σ_iso = **1.14 pp** — 2.5× wider than σ_default = 0.453 pp. Iso-tuned leader Δmeans: `sg_only_phi_budget` +1.16 pp (95% CI [−0.04, +2.30] pp — includes 0); `pair_gm_pdw` +1.59 pp (CI [+0.43, +2.62]); `slot_act_sine` +1.68 pp (CI [+0.49, +2.77]). All three Δs fall INSIDE the n=3 iso-tuned 2σ_iso = 2.28 pp band; they DO clear the n=7 default-config 2σ_default = 0.91 pp band. The Phase-5 ordinal gate min(leader) > max(baseline) **FAILS** for all three at iso-tuned n=3 (max baseline = 0.6057 from seed=1; min leaders 0.5998 / 0.6057 / 0.6039 ≤ 0.6057). **Honest framing: the default-config n=7 certification (the headline above) stands because σ_default is small at n=7; the iso-tuned-n=3 extension confirms directional positive Δ for all three winners but cannot itself re-certify at NeurIPS α. Phase-9f (n=7+ iso-tuned baseline-and-leader extension) would resolve this — at iso-tuned n=7 the Wilcoxon floor drops to 0.0078 and σ_iso would tighten by ~√(7/3) — and is filed as future work.** The `slot_act_sine` iso-tuned cell uses wd=2e-3 (not wd=5e-4); the wd=2e-3 baseline-neighbour extension is filed as Phase-9e.
...
- **Iso-tuned-cell extension at n=3 has wide baseline σ** (§5.5 iso-tuned paragraph, [`paper/STATISTICAL_TESTS.md`](paper/STATISTICAL_TESTS.md) §10). σ_iso at n=3 is 1.14 pp vs σ_default at n=7 = 0.453 pp; the iso-tuned Δs of +1.16 to +1.68 pp fall INSIDE 2σ_iso = 2.28 pp. The Phase-5 ordinal gate also fails at iso-tuned n=3. The directional signal is preserved across all three winners but cannot be formally certified at NeurIPS α at this n; Phase-9f n=7+ iso-tuned extension is the resolution path.
...
12. **Iso-tuned n=7+ extension** (Phase-9f). The 2026-05-31 baseline-extension landed n=3 at the iso-tuned (lr=3e-3, wd=5e-4, bs=128, AdamW) cell, surfacing σ_iso = 1.14 pp — too wide at n=3 to clear 2σ_iso. Extending baseline AND leaders to n=7+ at this cell (and at slot_act_sine's wd=2e-3 cell, Phase-9e) would deliver Wilcoxon floor 0.0078 in the iso-tuned regime and is the principled re-certification path; ~14 GPU-h.
