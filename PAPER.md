...
**DRAFT — POST-AUDIT REWRITE (2026-05-27).** The pre-audit version of this draft claimed "H09 phi_budget is the project's first defensible cross-dataset positive." That claim is **provisional pending the post-fix re-run** — the implementation-critic team discovered the network's realized stage-parameter ratio was 1:1.41:2.45, not the claimed 1:φ:φ². The Fixer campaign corrected this; the corresponding sweep rows are re-running on the corrected architecture as of commit `5442fdb`. This draft will be promoted to "final" only after the post-fix numbers land and survive a second elite-reviewer pass.
