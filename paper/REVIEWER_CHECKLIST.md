**ACCEPTANCE STATUS (2026-06-04 update): INTERNAL QA PASS at audit-calibration-centric framing with Phase-9i convergence-regime corrective binding landed — external review status: ICML 2027 mean 4.75 / 10 (Weak Reject main / Workshop Accept) prior to reframe.** This checklist is the project's *own* paper-acceptance gate; it does NOT override the independent external reviewer pass. **2026-06-04 update: the Phase-9i iso-modern-recipe + iso-convergence (200 ep) n=3 binding (H10 below) landed and CORRECTS the Phase-9h apparent refutation — all three priors LIFT the convergent baseline by +1.00 to +1.24 pp with 3/3 paired-positive + Phase-5 PASS for all three; the Phase-9h gap is correctly attributed to LR-tuning confound (apples-to-oranges), not prior failure. The protocol's headline is the self-falsification+correction *cycle*.** **2026-06-01 update: the PhD-grade critique landed and the paper has been reframed so that the audit-calibration result (Fisher p=1.94×10⁻⁵ at n=62; commit `e6f1f18`) is the headline empirical claim, with the self-falsification existence proof (Phase-9h tuned-baseline binding diagnostic: tuned baseline beats all three Phase-8 winners by +2.27 to +2.81 pp at Mann-Whitney p_one ∈ [0.0083, 0.0111]; `paper/STATISTICAL_TESTS.md` §14) as the methodological headline; **Phase-9i (`paper/STATISTICAL_TESTS.md` §15) corrects this to iso-recipe attribution.**** The three Phase-8 winners' default-config n=7 cert (Wilcoxon p=0.0078 < Holm α'=0.0167) is preserved as a matched-recipe empirical claim; the Phase-9i iso-modern + iso-convergence n=3 binding is preserved as a qualitatively-binding cross-recipe consistency check (NOT a formal NeurIPS-α cert at n=3; Phase-9j n≥7 filed as future work). The internal-QA findings are: all 42 Section A–G items PASS; Section H1–H3 hill-climb evidence landed 2026-05-30 PM; H9 Phase-9h tuned-baseline binding landed 2026-06-01 late evening; **H10 Phase-9i convergence-regime corrective binding landed 2026-06-04 morning**; Section I items I3 + I5 cleared after the dashboard refresh; new Section J audit-calibration items (J1–J3 below) cleared at the n=62 extension. **Camera-ready revision is in progress and tracked in `audits/ICML_REVIEWS_2026-05-30/REBUTTAL.md`.** The paper passes internal QA at the audit-calibration-centric framing with the Phase-9i corrective binding now in place; cross-domain replication, a Phase-9j n≥7 iso-modern formal-cert extension, and a true non-Claude external auditor are the remaining open work for the external review cycle.
...
>
- [x] **H8 (re-marked 2026-06-04).** Phase-9h tuned-baseline n=3 (convergence-regime context). *Phase-9h surfaced an apparent refutation: tuned baseline (n=3 mean 0.6017 at lr=0.01) BEAT all three priors' default-config n=7 means by +2.27 to +2.81 pp. Initial honest reading was "priors honestly demoted." 2026-06-04 Phase-9i correction (H10 below) re-attributes the gap to LR-tuning confound (apples-to-oranges: baseline got LR sweep, priors did not). Phase-9h stands as the load-bearing first-order self-falsification surface; the result is preserved verbatim as a methodology demonstration. See H9, H10 below for the corrective binding.*
...
- [x] **H10.** Phase-9i convergence-regime n=3 binding (iso-modern + iso-convergence; 2026-06-04 morning) — **HONEST RESULT: all 3 priors LIFT the convergent baseline by +1.00 to +1.24 pp; all 3 Phase-5 PASS; all 3 deliver 3/3 paired-positive; Phase-9h gap correctly attributed to LR-tuning confound, not prior failure.** *Phase-9i closed the iso-recipe + iso-convergence corrective binding for the Phase-9h diagnostic. All four arms (baseline + 3 priors) re-run at the modern 11-trick recipe (AdamW, cosine, label smoothing, RandAugment, MixUp/CutMix, EMA, etc.) at 200 ep CIFAR-100: convergent baseline n=3 mean = 0.6360 (σ=0.197 pp); `sg_only_phi_budget` n=3 mean = 0.6485 (σ=0.405 pp, **Δ=+1.24 pp**, Phase-5 PASS); `pair_gm_pdw` n=3 mean = 0.6460 (σ=0.067 pp, **Δ=+1.00 pp**, Phase-5 PASS); `slot_act_sine` n=3 mean = 0.6461 (σ=0.035 pp, **Δ=+1.01 pp**, Phase-5 PASS). Paired Wilcoxon W=0 p_one=0.125 for all three (n=3 floor; cannot clear Holm α'=0.0167 at this sample size). 95% paired-bootstrap CI (10 000 iter, rng=20260604): [+0.95, +1.43] / [+0.85, +1.08] / [+0.75, +1.17] pp — all three exclude 0 by ≥ +0.75 pp on the lower bound. 9/9 paired deltas across the three winners are strictly positive, ranging +0.75 to +1.43 pp. **Cross-regime synthesis is mutually consistent**: priors carry ~+1 pp of robust directional signal across default-config (Δ +1.24/+1.74/+1.78 pp at lr=1e-3 30 ep) AND modern-recipe (Δ +1.24/+1.00/+1.01 pp at modern 200 ep). The Phase-9h apparent refutation (H9 above) is correctly localised to LR-tuning confound, not prior failure. **Priors RESTORED from "demoted" (Phase-9h reading) to "screened candidates with consistent +1 pp directional lift across recipes; iso-modern-recipe formal cert pending n≥7."** Formal NeurIPS-α cert at iso-modern-recipe under Holm-Bonferroni α'=0.0167 requires n≥7 (Wilcoxon floor at n=3 is 0.125); filed as Phase-9j future work (~39 GPU-h on the 4090 Laptop). Full numbers: [`paper/STATISTICAL_TESTS.md`](STATISTICAL_TESTS.md) §15. Splice locations: PAPER.md §5.3 + abstract + §6.5 + §9 conclusion + §7 limitations; paper/FINDINGS.md 2026-06-04 MORNING PROMOTION block; this checklist H10.*
...
When all rows above are PASS, the paper can be promoted from DRAFT to FINAL. **Current state (2026-06-04 update): 42 of 42 Section A–G items PASS internally; Section H1–H5 hill-climb evidence landed; H9 Phase-9h tuned-baseline binding diagnostic landed; H10 Phase-9i convergence-regime corrective binding landed; Section I3 + I5 cleared; Section J1–J4 audit-calibration acceptance items PASS at n=62 Fisher p=1.94×10⁻⁵; J5 cross-family external-auditor item remains PARTIAL (8/10 strict CONCORDANT but in-family).** The reframe of 2026-06-01 promoted the audit-calibration result to the headline empirical claim; the 2026-06-04 Phase-9i corrective binding restored the three Phase-8 priors to "screened candidates with consistent +1 pp directional lift across default-config and modern-recipe regimes; iso-modern-recipe formal cert pending n≥7." The paper is therefore INTERNAL QA PASS at the audit-calibration-centric framing with the Phase-9i correction in place; the *external* gate is GATED by the prior-reframe ICML 2027 mean 4.75 / 10 verdict and is to be re-evaluated post-reframe and post-Phase-9i.

**The paper's headline empirical claim is now**: the protocol's MAJOR/BROKEN audit tier is statistically distinguishable from a clean-code floor at α=0.05 by ≈ 2500× (Fisher exact two-sided p=1.94×10⁻⁵; n=62 across 6 third-party codebases). **The paper's methodological headline** is the double-barrelled self-falsification + self-correction cycle (catch a: H09 12.6% realised-ratio drift; catch b two-sided: Phase-9h tuned-baseline beats all three winners by +2.27 to +2.81 pp → Phase-9i iso-modern-recipe corrective binding restores priors' +1 pp lift with 3/3 paired-positive + Phase-5 PASS for all three). The Phase-8 default-config n=7 cert (Wilcoxon p=0.0078 < Holm α'=0.0167) is preserved as a matched-recipe empirical claim; the Phase-9i iso-modern + iso-convergence n=3 binding (Δ +1.00 to +1.24 pp, 3/3 paired-positive) is preserved as a qualitatively-binding cross-recipe consistency check (NOT a formal NeurIPS-α cert at n=3; Phase-9j n≥7 filed as future work).
