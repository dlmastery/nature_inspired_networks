...
4. **H41 GoldenRatioAdamW's eps was φ-derived (1/φ⁴ ≈ 0.146).** The −32.82 pp falsification (51.96 % vs 84.78 % baseline) was REAL bad but for the WRONG REASON: at CIFAR gradient scales (~1e-3), the 0.146 eps dominated Adam's denominator, making effective LR ~6.85× nominal. The clean β-only experiment was never run — the falsification conflated two changes. **CORRECTION (2026-05-29, post-fix re-run + `audits/PAPER_GAP_G5.md`):** After Fixer-Opt restored stock `eps = 1e-8` and the β-only test ran, post-fix CIFAR-10 12-ep top-1 is **0.8394** (Δ ≈ −1 pp vs baseline 0.8478) — NOT a catastrophic falsification. The β-only Reddi-2018 non-convergence prediction is asymptotic; at the 12-ep screening horizon, β2=0.382 is only mildly damaging. The headline "clean β-only falsification" is **requalified to WEAKLY NEGATIVE (screening)**; a true Reddi-regime falsification needs 100+-epoch training and is deferred to the Phase-9 hill-climb at varied β2.
