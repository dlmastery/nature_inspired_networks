...
- 2026-06-01 — **Phase-9e Wave-1 measured (n=3 CIFAR-100 30-ep, default-config recipe).** Seeds 0/1/2 top1 = 0.5835 / 0.5802 / 0.5836, mean=0.5824, std(ddof=1)=0.0019. **Δ vs baseline_resnet20 default (0.5612, n=7): +2.12 pp** (95 % unpaired bootstrap CI [+1.78, +2.49] pp; Mann–Whitney U p_two=0.0167). **Δ vs `pair_gm_pdw` default (0.5786, n=7): only +0.38 pp** — paired n=3 (seeds 0/1/2) Δmean = +0.46 pp, 3/3 positive, exact paired Wilcoxon W=6 p_one=0.125 (at the n=3 floor; does NOT clear α=0.05). **VERDICT re-assessment: SUB-ADDITIVE.** The N=4 stack (pair_gm_pdw + slot_act_sine) outperforms baseline but does NOT meaningfully outperform the better solo winner. The Rule-23 prediction that "three orthogonal axes good, four orthogonal axes also good" is **empirically NOT supported at the screening compute budget**. The doctrine derived from n=1 screening data does NOT extend from 3 axes to 4 axes when tested at n=3. A Wave-2 extension to n=7 paired against `pair_gm_pdw` would resolve whether the +0.38–+0.46 pp increment is real or noise (filed as Phase-9e Wave-2). Full numbers: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §12; honest summary in [`paper/FINDINGS.md`](../../paper/FINDINGS.md) 2026-06-01 PM PROMOTION update and [`PAPER.md`](../../PAPER.md) §5.5 Wave-1 paragraph.
- 2026-06-01 (evening) — **Phase-9g Controls 1–4 cross-reference (the GPU pipeline closes here).** H87's "sub-additive N=4 stack" reading is now CONTEXTUALISED by Phase-9g Control 3a's tuned-baseline n=1 evidence: at lr=0.01 wd=5e-4 bs=256 AdamW the vanilla `baseline_resnet20` lands at 0.5984 single-seed — **+1.60 pp above H87's n=3 mean (0.5824) at default-config recipe**. The H87 N=4 stack outperforms its baseline_default reference (+2.12 pp) AND its better solo-winner reference (pair_gm_pdw default, +0.38 pp), but does NOT outperform a properly-tuned vanilla baseline at the n=1 hill-climb. The full Phase-9g picture (Control 1 φ-attribution partial refutation + Control 2 SIREN refutation + Control 3a tuned-baseline numerical superiority + Control 4 H71 inconclusive) tightens the main-track defensibility envelope: H87's status remains SUB-ADDITIVE at n=3 vs the better solo winner, and the H87+pair_gm_pdw cert pair both inherit the "default-config slice only" interpretation caveat from Phase-9g. Closure marker: [`audits/PIPELINE_COMPLETE_2026-06-01.md`](../../audits/PIPELINE_COMPLETE_2026-06-01.md). Cross-refs: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §13; [`PAPER.md`](../../PAPER.md) §5.5.6; [`paper/FINDINGS.md`](../../paper/FINDINGS.md) 2026-06-01 evening PROMOTION block. Phase-9h headline diagnostic: 3-seed re-run of `baseline_resnet20_tuned_lr0.01_wd0.0005` to resolve the n=1 vs n=7 asymmetry of Control 3a.
- 2026-06-01 (late evening) — **Phase-9h tuned-baseline n=3 binding cross-reference (the HEADLINE-DRIFT catch).** The Phase-9h 3-seed re-run of `baseline_resnet20_tuned_lr0.01_wd0.0005` (CIFAR-100 30 ep AdamW bs=256) landed at **n=3 mean = 0.6017 (σ = 0.31 pp)**, BEATING H87's `combo_n4_pair_slot` n=3 mean (0.5824) by **+1.93 pp** at the unpaired-comparison level. Combined with the Phase-9h tuned-baseline-vs-`pair_gm_pdw` Δ = +2.30 pp (Mann–Whitney p_one = 0.0083, no rank overlap) — i.e., the tuned vanilla baseline BEATS even the better solo winner H87 was stacked on — the H87 N=4 stack's already-sub-additive verdict is now further re-contextualised: **the N=4 stack does not even reach the tuned-vanilla-baseline performance level.** H87's status remains SUB-ADDITIVE at n=3 vs `pair_gm_pdw`, and the H87 stack's empirical claim (default-config) does NOT survive the iso-tuned-baseline binding diagnostic. The hypothesis is preserved as a worked example of the protocol's "sub-additive N=4 stack" finding; **the load-bearing empirical conclusion of the paper is now the protocol-as-meta-research-methodology, not the priors.** Closure marker (updated for Phase-9h binding): [`audits/PIPELINE_COMPLETE_2026-06-01.md`](../../audits/PIPELINE_COMPLETE_2026-06-01.md) §1.5 + §7 FINAL HEADLINE. Cross-refs: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §14; [`PAPER.md`](../../PAPER.md) §5.0 + abstract + §8 conclusion; [`paper/FINDINGS.md`](../../paper/FINDINGS.md) 2026-06-01 LATE EVENING block.
- 2026-06-04 (morning) — **Phase-9i convergence-regime n=3 binding (iso-modern + iso-200ep) — Phase-9h apples-to-oranges CORRECTED; H87 SOLO-COMPONENTS RESTORED.** Phase-9i re-ran all four arms (baseline + `pair_gm_pdw` + `slot_act_sine` + `sg_only_phi_budget`) at the *shared* modern 11-trick recipe (AdamW, cosine, label smoothing, RandAugment, MixUp/CutMix, EMA, …) at 200 ep CIFAR-100. The two solo components H87 is stacked on now LIFT the convergent modern-recipe baseline (n=3 mean 0.6360) by **+1.00 pp (`pair_gm_pdw` 0.6460)** and **+1.01 pp (`slot_act_sine` 0.6461)** at iso-modern + iso-convergence, with 3/3 paired-positive deltas + Phase-5 PASS for both — and `sg_only_phi_budget` by +1.24 pp on top of that. The Phase-9h apparent refutation is correctly attributed to **LR-tuning confound** (apples-to-oranges: baseline got LR sweep at lr=0.01; priors did not), NOT prior failure. H87's solo components are RESTORED as "screened candidates with consistent +1 pp directional lift across default-config (30 ep) and modern-recipe (200 ep) regimes" — and H87 itself, as the N=4 stack of those components, is now re-positioned as the **"sub-additive on top of two restored solo winners"** finding rather than the "sub-additive on top of a refuted baseline" finding. The H87 N=4 stack remains sub-additive at default-config-recipe-n=3 vs `pair_gm_pdw`; Phase-9i did NOT re-measure H87 at modern 200 ep (only baseline + the three solo Phase-8 winners ran). A Phase-9j extension that adds H87 to the modern 200-ep cell would resolve whether the N=4 stack remains sub-additive at iso-modern-recipe. Full Phase-9i numbers: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §15; PAPER.md splice: §5.3; paper/FINDINGS.md 2026-06-04 MORNING block; REVIEWER_CHECKLIST.md H10.
