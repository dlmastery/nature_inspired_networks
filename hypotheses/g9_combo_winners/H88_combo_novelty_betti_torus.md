...
- 2026-06-01 — **Phase-9e Wave-1 measured (n=3 CIFAR-100 30-ep, default-config recipe).** Seeds 0/1/2 top1 = 0.5353 / 0.5221 / 0.5307, mean=0.5294, std(ddof=1)=0.0067. **Δ vs baseline_resnet20 default (0.5612, n=7): −3.18 pp NEGATIVE** (95 % unpaired bootstrap CI [−3.89, −2.53] pp — excludes 0 on the negative side; Mann–Whitney U p_two=0.0167; all 3 combo seeds strictly below all 7 baseline seeds). **VERDICT re-assessment: EMPIRICALLY FALSIFIED.** The three novelty-pocket priors (H09 phi_budget + H22 toroidal + H51 Betti regularizer) stack DESTRUCTIVELY rather than productively, dropping 3.18 pp below baseline. The R-D-synthesis theoretical-orthogonality matrix predicted these three would compose because each touches a different forward-path layer (channel-budget / closure / topological regulariser); the prediction is empirically refuted. The pattern is consistent with the [`PAPER.md`](../../PAPER.md) §7.2.1 catastrophic `sg_full_fib` data point on CIFAR-10 (−11.54 pp from six-prior stack): novelty-pocket priors with no certified solo signal do NOT compose, even when chosen for orthogonality. This is one of the cleanest pieces of internal-replication negative evidence the campaign carries — it falsifies the doctrine that "theoretical orthogonality is sufficient for productive compounding" and converts that doctrine to the stricter empirical gate "compounding requires a certified solo winner per axis." H88 is **dead as a positive hypothesis**; preserved as a screened-negative case study with equal billing per Rule 9. Full numbers: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §12; honest summary in [`paper/FINDINGS.md`](../../paper/FINDINGS.md) 2026-06-01 PM PROMOTION update.
- 2026-06-01 (evening) — **Phase-9g Controls 1–4 cross-reference (the GPU pipeline closes here).** H88's EMPIRICALLY FALSIFIED verdict is independently reinforced by Phase-9g Control 1's finding that even a *certified* 3-axis stack (`pair_gm_pdw`) carries only ~+1.06 pp residual when φ-content is stripped (paired Δ vs nonphi only +0.61 pp at n=3, p_one=0.25). If `pair_gm_pdw`'s φ-attribution is weak, H88's H09-phi_budget + H22-toroidal + H51-Betti stack — which mixes φ-novelty with two untreated-orthogonal-axis priors that have NO certified solo signal — is even less likely to deliver a productive composition. Phase-9g Control 3a's tuned-baseline n=1 evidence (vanilla baseline at 0.5984 vs H88 at 0.5294) further widens the deficit: H88 falls **−6.90 pp below a properly-tuned baseline**, not just −3.18 pp below the default baseline. **The "novelty-pocket combos require certified solo signal per axis" gate is now triply reinforced** (Wave-1 H88 itself + Control 1 φ-residual weakness + Control 3a tuned-baseline numerical superiority). Closure marker: [`audits/PIPELINE_COMPLETE_2026-06-01.md`](../../audits/PIPELINE_COMPLETE_2026-06-01.md). Cross-refs: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §13; [`PAPER.md`](../../PAPER.md) §5.5.6; [`paper/FINDINGS.md`](../../paper/FINDINGS.md) 2026-06-01 evening PROMOTION block. No Phase-9h work proposed for H88 — the hypothesis is preserved as a screened-negative case study only.
- 2026-06-01 (late evening) — **Phase-9h tuned-baseline n=3 binding cross-reference.** The Phase-9h 3-seed re-run of `baseline_resnet20_tuned_lr0.01_wd0.0005` landed at **n=3 mean = 0.6017 (σ = 0.31 pp)**. The H88 combo n=3 mean (0.5294) falls **−7.23 pp below the tuned-vanilla baseline** (vs −3.18 pp below the default baseline). H88's EMPIRICALLY-FALSIFIED verdict is unchanged in direction but now widens substantially: the novelty-pocket combo loses to a properly-LR-tuned vanilla ResNet-20 by even more than it loses to the default baseline. The "novelty-pocket combos require certified solo signal per axis" gate (Rule 23 successor doctrine; derived from Wave-1) is now QUADRUPLY reinforced (Wave-1 H88 itself + Phase-9g Control 1 φ-residual weakness + Phase-9g Control 3a tuned-baseline numerical superiority + Phase-9h tuned-baseline n=3 binding). No Phase-9h+ work proposed for H88; it is preserved as a screened-negative case study with equal billing to the other DISCARDs per Rule 9. Closure marker: [`audits/PIPELINE_COMPLETE_2026-06-01.md`](../../audits/PIPELINE_COMPLETE_2026-06-01.md) §1.5 + §7. Cross-refs: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §14.
- 2026-06-04 (morning) — **Phase-9i convergence-regime n=3 binding cross-reference (Phase-9h apples-to-oranges corrected; H88 EMPIRICALLY-FALSIFIED verdict UNCHANGED).** Phase-9i restored the three solo Phase-8 priors at iso-modern-recipe + iso-convergence (200 ep CIFAR-100) — `sg_only_phi_budget` (one of the three H88 stack components) LIFTS the convergent baseline (0.6360) by +1.24 pp at modern 200 ep. H88's two other stack components (H22 toroidal and H51 Betti regularizer) have NO certified solo signal at any regime and were NOT extended to the modern recipe. **The Phase-9i correction does NOT rehabilitate H88**: the H09-phi_budget component now has a restored +1.24 pp lift at modern 200 ep, but stacking it with H22 + H51 (which carry no certified solo signal) gave −3.18 pp at default-config 30 ep. The "novelty-pocket combos require certified solo signal per axis" gate (derived from H88 Wave-1) is the load-bearing Rule-23 successor doctrine that H88 negatively contributed to the project. H88 remains an EMPIRICALLY-FALSIFIED screened-negative case study at default-config 30 ep; no Phase-9j extension is proposed for H88. Full Phase-9i numbers: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §15.
