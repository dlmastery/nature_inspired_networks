...
- 2026-06-01 — **Phase-9e Wave-1 measured (n=3 rotated_CIFAR-100 30-ep, default-config recipe).** Seeds 0/1/2 top1 = 0.4018 / 0.4025 / 0.4059, mean=0.4034, std(ddof=1)=0.0022. **VERDICT re-assessment: NOT EVALUABLE YET.** The absolute top1 of 0.40 on rotated_CIFAR-100 at 30 ep is consistent with the dataset's known difficulty without rotation-equivariant priors, but **no fair Δ is computable until a rotated_CIFAR-100 ResNet-20 baseline runs at the matched recipe.** The CIFAR-100 baseline (0.5612 on UNrotated CIFAR-100) is not a valid reference because the rotated task is harder (4 cardinal angles, all-4 TTA on eval) — the absolute-number gap is dataset difficulty, not prior signal. **Filed as Phase-9h future work: rotated_CIFAR-100 ResNet-20 baseline at the same default-config recipe (~1 GPU-h, ~3 seeds).** Until that lands, this hypothesis carries a tight n=3 measurement of the combo (Icosa-RoPE3D + fractal-recursion + group-avg-pool) on rotated_CIFAR-100 but NO comparison claim. Honest framing: this is the cleanest setup of the three Wave-1 combos (rotated task is the right testbed for icosahedral / rotation-equivariance priors per §5.7 verdict-on-the-wrong-testbed audit), but the baseline gap blocks the empirical claim. Full numbers: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §12.
- 2026-06-01 (evening) — **Phase-9g Control 4 (H71 IcosaRoPE3D direct ablation on rotated_CIFAR-10) — INCONCLUSIVE small positive.** The H71 IcosaRoPE3D positional encoding (the central novelty of this combo) was tested in direct ablation form on rotated_CIFAR-10 (ViT-Tiny depth=12 embed=198) against a 1D-RoPE comparator on the same scaffold: `h71_icosa_rope3d_vit_tiny_rotcifar10` mean=0.6525 (n=3, seeds 0.6534/0.6484/0.6555) vs `vit_tiny_1d_rope_rotcifar10`=0.6507 (n=1). **Δ = +0.18 pp** — small positive but the 1D-RoPE comparator at n=1 sits inside the IcosaRoPE3D ±1σ band (σ_IcosaRoPE3D = 0.37 pp), so the IcosaRoPE3D-specific advantage is NOT statistically certified. **This is the first empirical evidence on H91's primary novelty (icosahedral 3D RoPE) on its proper testbed (rotated dataset + ViT scaffold), and it is INCONCLUSIVE.** The IcosaRoPE3D positional encoding does not currently demonstrate a rotation-equivariance advantage over standard 1D RoPE at the n=3-vs-n=1 screening budget. H91 status remains NOT EVALUABLE YET pending (a) Phase-9h rotated_CIFAR-100 ResNet-20 baseline + (b) Phase-9h 3-seed extension of `vit_tiny_1d_rope_rotcifar10` to compute a fair paired comparison on the H71 mechanism. Closure marker: [`audits/PIPELINE_COMPLETE_2026-06-01.md`](../../audits/PIPELINE_COMPLETE_2026-06-01.md). Cross-refs: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §13.4; [`PAPER.md`](../../PAPER.md) §5.5.6 Control 4 bullet; [`paper/FINDINGS.md`](../../paper/FINDINGS.md) 2026-06-01 evening PROMOTION block. The "NOVEL+TESTABLE research proposal" framing of H71 (and H91 by extension) is preserved.
- 2026-06-01 (late evening) — **Phase-9h tuned-baseline n=3 binding cross-reference.** The Phase-9h binding diagnostic landed for CIFAR-100 (unrotated), not rotated_CIFAR-100, so it does NOT directly resolve H91's "no fair Δ until rotated baseline lands" verdict. H91 status remains **NOT EVALUABLE YET** — the binding for H91's empirical claim is still (a) Phase-9h+ rotated_CIFAR-100 ResNet-20 baseline at the matched recipe + (b) Phase-9h+ 3-seed extension of `vit_tiny_1d_rope_rotcifar10` for a fair paired Wilcoxon on the H71 mechanism. However, the Phase-9h CIFAR-100 result independently strengthens the paper's headline-interpretation shift: at iso-tuned conditions on unrotated CIFAR-100, the priors do NOT robustly survive a properly-LR-tuned baseline; by extension the rotated-task evaluation is even less likely to robustly favour rotation-equivariant priors at the screening compute budget once the rotated baseline lands at matched recipe and matched LR tuning. H91 is preserved as a NOT-EVALUABLE-YET hypothesis whose evaluation is now part of the Phase-9i+ rotated-task closure (not blocking the workshop / D&B-track submission). The H71 IcosaRoPE3D "research proposal" framing is preserved. Closure marker: [`audits/PIPELINE_COMPLETE_2026-06-01.md`](../../audits/PIPELINE_COMPLETE_2026-06-01.md) §1.5 + §7. Cross-refs: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §14.
- 2026-06-04 (morning) — **Phase-9i convergence-regime n=3 binding cross-reference (Phase-9h apples-to-oranges corrected on unrotated CIFAR-100; H91 NOT EVALUABLE YET UNCHANGED on rotated_CIFAR-100).** Phase-9i restored the three solo Phase-8 priors at iso-modern-recipe + iso-convergence (200 ep CIFAR-100) — the priors LIFT the convergent baseline by +1.00 to +1.24 pp at iso-recipe + iso-convergence on unrotated CIFAR-100. **For H91 (rotated_CIFAR-100 task), Phase-9i does NOT close the rotated-baseline gap**: the iso-modern-recipe + iso-200ep sweep ran ONLY on unrotated CIFAR-100. H91's empirical claim continues to require (a) a rotated_CIFAR-100 ResNet-20 baseline at the matched recipe + (b) the 3-seed `vit_tiny_1d_rope_rotcifar10` extension. **Updated interpretation of the Phase-9h CIFAR-100 result**: the Phase-9h "tuned baseline beats priors" reading was apples-to-oranges (asymmetric LR sweep), now correctly attributed to LR-tuning confound by Phase-9i — so the corollary "rotated-task evaluation is unlikely to favour rotation-equivariant priors once the rotated baseline is LR-tuned" weakens substantially. The honest residual reading is "H91's rotated-task evaluation remains the cleanest setup for the H71 IcosaRoPE3D research proposal; iso-recipe + iso-convergence on rotated_CIFAR-100 is the principled binding." H91 remains NOT EVALUABLE YET; filed as Phase-9j rotated-task closure work. Full Phase-9i numbers: [`paper/STATISTICAL_TESTS.md`](../../paper/STATISTICAL_TESTS.md) §15.
