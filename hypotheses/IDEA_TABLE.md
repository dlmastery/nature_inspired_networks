...
| 81 | **H81** | H177 Vibration | `SinusoidalActivation` | SIREN-style `sin(ωx)`, learnable ω (Sitzmann 2020) | 🏆 **CERTIFIED n=7 (2026-05-29)** — `slot_act_sine` (SLOT variant) CIFAR-100 30-ep Δmean **+1.78 pp**, paired Wilcoxon **p=0.0078**, Holm-Bonferroni α'=0.0167 cleared (k=3 family); promoted SCREENING→EVALUATION per Rule 28. CIFAR-10 12-ep sweep row `sg_only_sine_act` retained as screening. ⛰️ **hill-climbed +2.08 pp** (Phase-9a, 2026-05-30, n=3 tuned-vs-tuned). | `src/nature_inspired_networks/sinusoidal_activation.py` |
