...
| 8 | **H08** | Dynamic φ-Growth — adaptive scaling: add layers during training following Fib rule | **FALSIFIED_AT_MECHANISM_TEST** (G1 audit: Fibonacci schedule indistinguishable from any progressive-growth schedule; bespoke composite formula violates Rule 2) | `src/nature_inspired_networks/dynamic_growth.py` | Implement layer-add callback; measure cumulative epoch cost to target | `ideas/08_dynamic_phi_growth/` |
...
| 43 | **H43** | Fibonacci Pruning — iterative pruning ratios follow Fib | **FALSIFIED_AT_MECHANISM_TEST** (G5 sci-critic: Fibonacci-anchored prune-rate schedule converges to constant 1/φ after k=4, operationally indistinguishable from constant-0.618 IMP per Han 2015 / Frankle & Carbin 2018) | `src/nature_inspired_networks/pruning.py` | Prune {8,13,21,34,55}% of weights iteratively; vs. magnitude+global | `ideas/43_fib_pruning/` |
...
| 67 | **H67** | **Full Paradigm Hybrid** — flagship Sacred-Liquid-JEPA-KAN-GNN-Transformer fusion | **FALSIFIED_AT_MECHANISM_TEST** (G7 audit BROKEN finding: half-on stress test with hardcoded `which_priors_active=True`, LiquidCFC collapsed to affine + nonlinearity; sci-critic UNFALSIFIABLE due to 10-prior attribution-impossibility) | `src/nature_inspired_networks/hybrid_full.py` | Compose all paradigm bridges; ablate component-by-component | `ideas/67_full_paradigm_hybrid/` |
...
| 74 | **H74** | Metatron Overlap Tying — single tied conv weight + 13 learnable circle scales | **FALSIFIED_AT_MECHANISM_TEST** (G7 audit BROKEN finding: MetatronTiedConv2d's 13 alphas Σ-summed to a single scalar gate; sci-critic DERIVATIVE+TESTABLE — Metatron-basis is decorative on top of a low-rank tied factorisation à la LoRA shared-base) | `src/nature_inspired_networks/hybrid_metatron_tying.py` | Weight-tied Metatron kernel with per-circle scales | `ideas/74_metatron_overlap_tying/` |
