"""H22 - idea-specific experiment driver.

Thin wrapper that loads a YAML config from ``configs/`` and calls the
project runner. The experiment archive is written under
``experiments/<tag>/run_seed<S>/`` so each archived sub-directory is
self-contained per CLAUDE.md rule #8.
"""
