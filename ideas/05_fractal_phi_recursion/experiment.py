"""H05 — idea-specific experiment driver.
...
from implementation import idea_signature, predicted_param_factor  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Run one experiment for H05.")
    ap.add_argument("--config", required=True,
                    help="Path to config YAML in this idea's configs/ dir.")
    ap.add_argument("--tag", required=True,
                    help="Run tag, also used as archive sub-dir name "
                         "(e.g., exp001_fractal_d2).")
    ...
    depth = cfg.get("fractal_depth", 2)
    print(f"[h05] idea_signature={idea_signature()}")
    print(f"[h05] predicted param-blow-up factor at depth={depth}: "
          f"{predicted_param_factor(depth):.2f}x")
