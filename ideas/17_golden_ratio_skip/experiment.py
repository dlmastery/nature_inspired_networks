"""H17 — idea-specific experiment driver.
...
from implementation import PhiSkipBlock, idea_signature  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Run one experiment for H17.")
    ...
    mode = cfg.get("phi_skip_mode", "inv_phi")
    if mode not in PhiSkipBlock.VALID_MODES:
        raise ValueError(
            f"config phi_skip_mode={mode!r} not in {PhiSkipBlock.VALID_MODES}"
        )

    print(f"[h17] idea_signature={idea_signature()}")
    print(f"[h17] phi_skip_mode={mode}")
