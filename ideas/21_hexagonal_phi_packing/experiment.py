"""H21 — idea-specific experiment driver.
...
from implementation import idea_signature, variance_preserving_gain  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Run one experiment for H21.")
    ...
    print(f"[h21] idea_signature={idea_signature()}")
    print(f"[h21] variance_preserving_gain(k=3) = "
          f"{variance_preserving_gain(3):.4f}")
    if cfg.get("apply_gain"):
        print("[h21] He-init gain compensation ENABLED in config")
    else:
        print("[h21] He-init gain compensation DISABLED (hypothesis-doc default)")
