...
| 9 | **H09** | Golden Proportion Parameter Budget — total params allocated 1 : φ : φ² : … across stages | ✓ done (`phi_budget` .8554 — **only variant beating baseline**) | `src/nature_inspired_networks/phi_scaling.py:phi_budget_widths` / `PhiBudgetNet` | Explicit param-budget allocator that respects total target budget B | `ideas/09_golden_param_budget/` |
...
| 44 | **H44** | Golden Regularization — L1/L2 coef = 1/φ^k per layer | ✓ done (`phi_decay` .7981) | `src/nature_inspired_networks/phi_decay.py` | Per-layer weight decay 1/φ^depth; CIFAR + over-fitting test | `ideas/44_golden_regularization/` |
...
| 48 | **H48** | Golden Momentum Scheduler — momentum decay φ^{-epoch} | ✓ done (`golden_momentum` .8352) | `src/nature_inspired_networks/schedulers.py` | β1 schedule from 1/φ to 1/φ²; vs. constant 0.9 | `ideas/48_golden_momentum/` |
